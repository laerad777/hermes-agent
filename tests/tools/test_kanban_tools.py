...
def test_show_includes_comment_id_for_exact_read_back(worker_env):
    from tools import kanban_tools as kt

    commented = json.loads(
        kt._handle_comment({"task_id": worker_env, "body": "durable evidence"})
    )
    shown = json.loads(kt._handle_show({}))

    assert commented["comment_id"] > 0
    assert {
        "id": commented["comment_id"],
        "body": "durable evidence",
    }.items() <= shown["comments"][-1].items()


def _make_typed_review_worker(worker_env, *, role="critic"):
    from hermes_cli import kanban_db as kb

    conn = kb.connect()
    try:
        conn.execute(
            "UPDATE tasks SET workflow_template_id=?, current_step_key=?, assignee=? "
            "WHERE id=?",
            ("jerome-kanban-v1", role, role, worker_env),
        )
        conn.execute(
            "UPDATE task_runs SET profile=? WHERE task_id=? AND ended_at IS NULL",
            (role, worker_env),
        )
        conn.commit()
        ...
        kb.bind_review_run_authority(conn, worker_env, workspace)
        os.environ["HERMES_KANBAN_RUN_ID"] = str(task.current_run_id)
    finally:
        conn.close()


def test_typed_review_tool_rejects_empty_metadata_despite_worker_session_stamp(
    worker_env, monkeypatch
):
    from hermes_cli import kanban_db as kb
    from tools import kanban_tools as kt

    _make_typed_review_worker(worker_env)
    monkeypatch.setenv("HERMES_PROFILE", "critic")
    monkeypatch.setenv("HERMES_SESSION_ID", "typed-review-session")

    rejected = json.loads(kt._handle_complete({"summary": "OKAY", "metadata": {}}))

    assert "review provenance" in rejected["error"]
    conn = kb.connect()
    try:
        task = kb.get_task(conn, worker_env)
...
def test_typed_review_provenance_survives_tool_run_event_show_and_child_context(
    worker_env, monkeypatch
):
    from hermes_cli import kanban_db as kb
    from tools import kanban_tools as kt
    ...
    monkeypatch.setenv("HERMES_SESSION_ID", "typed-review-session")
    commented = json.loads(
        kt._handle_comment({"task_id": worker_env, "body": "full review evidence"})
    )
    shown_before = json.loads(kt._handle_show({}))
    assert shown_before["comments"][-1]["id"] == commented["comment_id"]
    conn = kb.connect()
    try:
        authority = kb.get_review_run_authority(
            conn, kb.get_task(conn, worker_env).current_run_id
        )
        ...
        conn.close()
    metadata = {
        "durable_comment_id": commented["comment_id"],
        "durable_comment_read_back": True,
        "inspected_revision": inspected_revision,
        "inspected_symbols": ["hermes_cli.kanban_db.complete_task"],
        "red_tests": ["session-only metadata rejects"],
    }
    expected = json.loads(json.dumps(metadata))

    completed_out = json.loads(
        kt._handle_complete({"summary": "OKAY", "metadata": metadata})
    )

    assert completed_out["ok"] is True
    assert metadata == expected
    conn = kb.connect()
    try:
        child = kb.create_task(
            conn, title="consume review", assignee="worker", parents=[worker_env]
        )
        ...
        persisted = {**expected, "worker_session_id": "typed-review-session"}
        assert run.metadata is not None
        assert {key: run.metadata[key] for key in persisted} == persisted
        ...
        assert shown["runs"][-1]["metadata"] == run.metadata
