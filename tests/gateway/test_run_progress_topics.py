...
class FailedTransformedEditProgressCaptureAdapter(MetadataEditProgressCaptureAdapter):
    async def edit_message(
        ...
        if "[plugin appended this]" in content:
            return SendResult(success=False, error="edit rejected", retryable=False)
        return SendResult(success=True, message_id=message_id)
...
@pytest.mark.asyncio
async def test_failed_transformed_edit_leaves_normal_send_authoritative(monkeypatch, tmp_path):
    adapter, result = await _run_with_agent(
        monkeypatch,
        tmp_path,
        TransformedStreamAgent,
        ...
        chat_type="group",
        thread_id="$thread",
        adapter_cls=FailedTransformedEditProgressCaptureAdapter,
    )
